...
def _zenodo_record() -> dict[str, Any]:
 try:
  req=Request(_ZENODO_API,headers={"Accept":"application/json","User-Agent":"testHP-reference-tissue/4.0"})
...
def reference_tissue_manifest(source_id:str)->dict[str,Any]:
 ...
 return {"source":source,"dataLoadStatus":"local_extract_available" if local else "manifest_only","tissueIds":[],"spatialCoordinates":[],"transform":None,"localExtractAvailable":local,"localExtractPath":str(_LOCAL_PREVIEW.relative_to(_ROOT)) if local else None,"note":"Large spatial data are not fetched automatically. A verified transform into the NIH hand-template frame is not established."}
...
def reference_tissue_cells_preview(source_id:str,region:str=Query(default="palm"),limit:int=Query(default=25,ge=1,le=100))->dict[str,Any]:
 ...
 if source_id=="human-skin-spatial-census" and _LOCAL_PREVIEW.is_file():
  try:
   ...
   return {"sourceId":source_id,"status":"bounded_local_cell_preview","region":region,"requestedLimit":limit,"returnedCount":len(cells),"cells":cells,"coordinateScope":"sample_local","registrationStatus":"unregistered_to_hand","transform":None,"matrixLoaded":False,"dataLoaded":True,"localExtract":True,"sourceFile":payload.get("sourceFile"),"sourceCellCount":payload.get("sourceCellCount"),"note":"Cells are served from a locally materialized bounded extract. Coordinates remain in dataset/sample-local space and are not projected onto NIH hand geometry."}
