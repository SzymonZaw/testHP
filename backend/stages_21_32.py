"""Research-only contracts for Digital Twin stages 21-32."""
...
class Evidence(BaseModel):
 source:str; modality:str|None=None; dataset_id:str|None=None; timepoint:str|None=None; spatial_node_id:str|None=None
 quality:float|None=Field(default=None,ge=0,le=1); confidence:float|None=Field(default=None,ge=0,le=1)
class DerivedResult(BaseModel):
 subject_id:str; timepoint:str|None=None; estimate:float|None=None; unit:str|None=None
 uncertainty:dict[str,Any]=Field(default_factory=dict); evidence:list[Evidence]=Field(default_factory=list); status:str="research_only"
 created_at:str=Field(default_factory=lambda:datetime.now(timezone.utc).isoformat())
class TrajectoryRequest(BaseModel): subject_id:str; observations:list[DerivedResult]=Field(default_factory=list)
def register_stage_21_32_routes(app:Any)->None: app.include_router(router)
def stage_catalog()->dict[str,Any]: return {"schema":SCHEMA,"stages":STAGES}
def build_biological_age(result:DerivedResult)->dict[str,Any]:
 return {**result.model_dump(),"stage":21,"status":"evidence_backed" if result.estimate is not None and result.evidence else ("not_established" if result.estimate is None else "insufficient_evidence")}
def build_trajectory(req:TrajectoryRequest)->dict[str,Any]:
 ordered=[x for x in req.observations if x.timepoint]
 return {"schema":SCHEMA,"stage":22,"subject_id":req.subject_id,"timepoints":[x.timepoint for x in ordered],"observations":[x.model_dump() for x in ordered],"status":"research_only" if len(ordered)>=2 else "insufficient_timepoints","uncertainty":{"reason":"trajectory requires repeated observations"}}
def build_unified_twin(subject_id:str,records:list[dict[str,Any]])->dict[str,Any]:
 return {"schema":SCHEMA,"stage":24,"subject_id":subject_id,"scales":["surface","anatomy","tissue","cell","molecular"],"records":records,"coordinate_system":"HAND_COORDINATE_SYSTEM","requires_lineage":True,"status":"research_only"}
@router.get('/api/hand/stages-21-32')
def get_stages_21_32(): return stage_catalog()
@router.post('/api/hand/biological-age')
def biological_age(payload:DerivedResult): return build_biological_age(payload)
@router.post('/api/hand/aging-trajectory')
def aging_trajectory(payload:TrajectoryRequest): return build_trajectory(payload)
@router.post('/api/hand/unified-twin')
def unified_twin(payload:dict[str,Any]): return build_unified_twin(str(payload.get('subject_id') or ''),list(payload.get('records') or []))
