...
from .data_ingestion import ROOT, ingest_upload, registry_status, safe_component
...
def normalize_view(value: str | None) -> str | None:
    ...
    return raw if raw in SUPPORTED_VIEWS else None


def infer_view(filename: str, explicit: str | None = None) -> str | None:
    """Resolve explicit metadata first, then a conservative filename token."""
    explicit_view = normalize_view(explicit)
    if explicit_view:
...
def _crop_box(mask: np.ndarray) -> tuple[int, int, int, int]:
    ...
    return (max(0, int(xs.min()) - pad_x), max(0, int(ys.min()) - pad_y), min(mask.shape[1], int(xs.max()) + pad_x + 1), min(mask.shape[0], int(ys.max()) + pad_y + 1))
...
def prepare_image(record: dict[str, Any]) -> dict[str, Any]:
    ...
    rgb = rgba[:, :, :3]
    alpha = rgba[:, :, 3] if source_has_alpha else None
    ...
    record.update({
        ...
        "background_method": "embedded-alpha" if source_has_alpha else "adaptive-border-separation",
        "updated_at": utc_now(),
    })
    return record
...
def state(subject_id: str, timepoint: str) -> dict[str, Any]:
    ...
    return {"schema": "photo-reconstruction-stages-1-5", "subject_id": safe_component(subject_id, "subject"), "timepoint": safe_component(timepoint, "T0"), "views": list(SUPPORTED_VIEWS), "inputs": records, "assigned_count": len(assigned), "prepared_count": len(prepared), "ready_views": sorted({x["view"] for x in prepared}), "minimum_views": 2, "can_register": len(prepared) >= 2, "can_reconstruct": len(prepared) >= 2}
...
def register_prepared(subject_id: str, timepoint: str) -> dict[str, Any]:
    ...
    for record in records:
        ...
        if len(landmarks) < 4:
            record["registration"] = {"status": "needs-review", "method": detected.get("method", "none"), "landmarks": len(landmarks), "quality": 0.0, "warning": "Hand landmarks were not detected reliably; review this view before reconstruction."}
        ...
        record["registration"] = {"status": "registered", "method": detected.get("method", "unknown"), "landmarks": normalized_landmarks, "landmark_count": len(landmarks), "bbox": bbox, "transform": {"origin": [bbox["min_x"], bbox["min_y"]], "scale": round(scale, 6)}, "coordinate_system": "hand-surface-v1", "quality": round(min(1.0, len(landmarks) / 21.0), 4), "registered_at": utc_now()}
