...
from core.spatial_scope import canonical_parent_id, split_spatial_scope
...
def _location_parent(payload:dict[str,Any],spatial_id:str)->str|None:return str(payload["parent_id"]) if payload.get("parent_id") else canonical_parent_id(spatial_id)
...
def _parse_observations(items:list[dict[str,Any]])->tuple[list[Observation],list[AnatomicalLocation]]:
    ...
    for item in items:
        ...
        location=AnatomicalLocation(id=spatial_id,name=str(item.get("location_name") or spatial_id.rsplit("/",1)[-1]),level=str(item.get("location_level") or "site"),parent_id=_location_parent(item,spatial_id))
        observations.append(Observation(id=str(item["id"]),subject_id=str(item["subject_id"]),timepoint_id=str(item["timepoint"]),name=str(item["name"]),value=item.get("value"),observed_at=datetime.fromisoformat(str(item["observed_at"]).replace("Z","+00:00")),anatomical_location=location,source_measurement_ids=list(item.get("source_measurement_ids") or []),metadata={"validated_interpretations":item.get("validated_interpretations") or {},"biological_level":item.get("biological_level","unspecified")},biological_level=str(item.get("biological_level") or "unspecified"),modality=str(item.get("modality") or "unknown"),status=str(item.get("status") or "active"),version=int(item.get("version") or 1)))
        locations[spatial_id]=location
    pending=list(locations.values())
    while pending:
        location=pending.pop()
        if not location.parent_id or location.parent_id in locations:continue
        parent_id=location.parent_id
        parent=AnatomicalLocation(id=parent_id,name=parent_id.rsplit("/",1)[-1].replace("_"," ").title(),level="site" if "/" not in parent_id else "anatomical_region",parent_id=canonical_parent_id(parent_id))
        locations[parent.id]=parent;pending.append(parent)
...
def _evidence(items:list[dict[str,Any]])->list[Evidence]:
    ...
    for item in items:
        ...
        result.append(Evidence(id=str(evidence_id),subject_id=str(item["subject_id"]),observation_id=str(item["id"]),evidence_type=str(item.get("evidence_type") or "source"),interpretation_boundary="observation_only",provenance={"source":item.get("source","manual-entry")},confidence=float(confidence) if confidence is not None else None))
    return result
...
def _build_state(subject_id:str,timepoint:str,spatial_id:str|None,include_descendants:bool):
    ...
    observations,locations=_parse_observations(items);evidence=_evidence(items);aggregator=BiologicalStateAggregator(observations,evidence,locations)
    if spatial_id is None:
        scoped_observations=[o for o in observations if o.subject_id==subject_id and o.timepoint_id==timepoint];scoped_evidence=tuple(evidence);state=aggregator.build_state(subject_id,timepoint)
    else:
...
def biological_state(subject_id:str="own_cohort",timepoint:str="T0",spatial_id:str|None=None,include_descendants:bool=True):
    ...
    direct_evidence=tuple(item for item in scoped_evidence if spatial_id is None or next((o for o in scoped_observations if o.id==item.observation_id and o.anatomical_location and o.anatomical_location.id==spatial_id),None) is not None);direct_ids={item.id for item in direct_evidence};descendant_evidence=tuple(item for item in scoped_evidence if item.id not in direct_ids)
    if spatial_id is None:direct_observations=list(scoped_observations);descendant_observations=[]
    else:
        direct_items,descendant_items=split_spatial_scope(all_items,spatial_id,include_descendants=include_descendants);direct_ids_observations={str(item["id"]) for item in direct_items};descendant_ids_observations={str(item["id"]) for item in descendant_items};direct_observations=[o for o in scoped_observations if o.id in direct_ids_observations];descendant_observations=[o for o in scoped_observations if o.id in descendant_ids_observations]
    by_location={};by_level={level:0 for level in _LEVELS};direct_by_level={level:0 for level in _LEVELS};descendant_by_level={level:0 for level in _LEVELS}
    for observation in scoped_observations:
        ...
        if level in by_level:by_level[level]+=1
        location=observation.anatomical_location
        if not location:continue
        entry=by_location.setdefault(location.id,{"spatial_id":location.id,"name":location.name,"count":0,"by_level":{item:0 for item in _LEVELS}});entry["count"]+=1
        if level in _LEVELS:entry["by_level"][level]+=1
    ...
    return {"state":_state_payload(state,editable,len(scoped_observations)),"summary":{"scope":spatial_id,"include_descendants":include_descendants,"observations":len(scoped_observations),"observation_count":len(scoped_observations),"data_count":len(scoped_observations),"state_count":state.evidence_count,"explicit_evidence":state.evidence_count,"direct_evidence":len(direct_evidence),"descendant_evidence":len(descendant_evidence),"direct_observations":len(direct_observations),"descendant_observations":len(descendant_observations),"by_level":by_level,"direct_by_level":direct_by_level,"descendant_by_level":descendant_by_level,"by_location":sorted(by_location.values(),key=lambda item:item["spatial_id"]),"interpretation_source":"validated_interpretations only","dimensions":_DIMENSIONS,"scope_definition":"spatial_id exact match plus recursive descendants; biological_level is a separate dimension"}}
