...
_canonical_parent_id = canonical_parent_id
...
def _build_state(subject_id: str, timepoint: str, spatial_id: str | None, include_descendants: bool):
    ...
    if spatial_id is None:
        scoped_observations = [o for o in observations if o.subject_id == subject_id and o.timepoint_id == timepoint]
        ...
        scoped_ids = {str(item["id"]) for item in direct_items + descendant_items}
        scoped_observations = [o for o in observations if o.id in scoped_ids]
...
def biological_state(subject_id: str = "own_cohort", timepoint: str = "T0", spatial_id: str | None = None, include_descendants: bool = True):
    ...
    if spatial_id is None:
        direct_observations = list(scoped_observations); descendant_observations: list[Observation] = []
    else:
    ...
    by_level = {level: 0 for level in _LEVELS}; direct_by_level = {level: 0 for level in _LEVELS}; descendant_by_level = {level: 0 for level in _LEVELS}
