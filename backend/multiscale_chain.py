...
The chain is deliberately a validation/query contract. It does not diagnose
 disease or prescribe treatment; interpretation remains an explicit, versioned
assessment with evidence and uncertainty.
"""
...
class MultiscaleChain:
    """One coherent observation/interpretation path from hand anatomy to cell."""
    ...
    def to_dict(self) -> dict[str, Any]:
        """Return a JSON-serializable hierarchy after validating every link."""
        self.validate()
        result: dict[str, Any] = {
            "context": {
                "subject_id": self.anatomy.subject_id,
                "hand_id": self.anatomy.hand_id,
                "timepoint_id": self.anatomy.timepoint_id,
            },
            "anatomy_id": self.anatomy.structure_id,
            "tissue_id": self.tissue.tissue_id,
            "histology_id": self.histology.histology_id if self.histology else None,
            "cell_id": self.cell.cell_id if self.cell else None,
            "state_assessment_id": self.state_assessment.assessment_id if self.state_assessment else None,
            "age_estimate_id": self.age_estimate.estimate_id if self.age_estimate else None,
        }
        return result


def build_multiscale_chain(
    anatomy: AnatomicalStructure,
    tissue: TissueRegion,
    *,
    histology: HistologyRegion | None = None,
    ...
    """Construct and validate a complete or partial multiscale chain."""
    chain = MultiscaleChain(
        anatomy=anatomy,
        tissue=tissue,
        histology=histology,
        cell=cell,
