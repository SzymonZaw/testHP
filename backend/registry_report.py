...
def build_registry_report(registry: MultiscaleRegistry, *, subject_id: str, hand_id: str, timepoint_id: str, longitudinal_observations: Iterable[Any] | None = None) -> dict[str, Any]:
    ...
    ages = [x["age_estimate"] for x in canonical_cells if x["age_estimate"] is not None]
    ...
    attention = build_attention_map([{"zone_id": x["zone"], "level": "cell", "metric": x["metric"], "cell_count": 1, "changed_cells": 1 if x.get("status") == "observed_change" else 0, "mean_delta": x.get("delta")} for x in trends if x.get("status") != "insufficient_timepoints"])
