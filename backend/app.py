...
app = FastAPI(title="Human Pathology Platform", version="0.8.1")

class PipelineRequest(BaseModel): datasets: list[str] = []
...
def dataset_registry() -> list[dict[str, Any]]:
    cfg = load_config().get("datasets", {}); registry: list[dict[str, Any]] = []
    for modality, entries in (("image", cfg.get("images", {})), ("wsi", cfg.get("wsi", {})), ("rna", cfg.get("rna", {})), ("hand", cfg.get("hand", {}))):
        for name, spec in entries.items():
            ...
            path = ROOT / path_value; files = iter_files(path); enabled = bool(spec.get("enabled", True)); formats = set(spec.get("formats") or spec.get("image_formats") or [])
            ...
            supported = [p for p in files if p.suffix.lower() in {x.lower() for x in formats}]; empty = [p for p in supported if p.stat().st_size == 0]
...
def managed_as_pipeline_dataset(item: dict[str, Any]) -> dict[str, Any]:
    return {"name":item["name"],"modality":item["modality"],"task":item.get("description") or "managed research dataset","path":item["root_path"],"exists":True,"enabled":True,"files":item.get("file_count",0),"supported_files":item.get("available_files",0),"bytes":sum(x.get("size_bytes",0) for x in refresh_manifest(item["dataset_id"])["records"]),"empty_files":item.get("empty_files",0),"available":item.get("available_files",0)>0,"reason":None,"description":item.get("description", ""),"source":item.get("source", ""),"version":item.get("version", "1.0"),"license":item.get("license"),"tags":item.get("tags", []),"dataset_id":item["dataset_id"]}
...
def run_pipeline(selected: list[str]) -> dict[str, Any]:
    registry = dataset_registry(); managed = [managed_as_pipeline_dataset(x) for x in list_datasets(registry)]
    by_name = {x["name"]: x for x in registry}
    for item in managed: by_name.setdefault(item["name"], item)
    chosen = selected or list(by_name); missing = [name for name in chosen if name not in by_name]
    validations = {name:validate_dataset(by_name[name]) for name in chosen if name in by_name}; valid_items=[x for x in validations.values() if x["valid"]]; warnings=[w for item in validations.values() for w in item["warnings"]]
...
def datasets(): return {"raw_exists":RAW_ROOT.exists(),"datasets":[validate_dataset(x) for x in dataset_registry()],"registry":list_datasets(dataset_registry())}
...
async def upload_to_dataset(dataset_id: str,file: UploadFile=File(...),subject_id: str=Form("unknown"),timepoint: str=Form("unknown"),subtype: str|None=Form(None),view: str|None=Form(None)):
    item=get_dataset(dataset_id,dataset_registry())
    if item is None: raise HTTPException(status_code=404,detail="dataset not found")
    filename=safe_component(file.filename or "upload.bin","upload.bin"); target=ROOT/item["root_path"]/safe_component(subject_id,"subject")/safe_component(timepoint,"T0")
    if subtype: target=target/safe_component(subtype,"subtype")
    target.mkdir(parents=True,exist_ok=True); destination=target/filename
    ...
    content=await file.read(); destination.write_bytes(content); manifest=refresh_manifest(dataset_id)
