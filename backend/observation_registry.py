...
SPATIAL_ALIASES = {"palm": "hand/palm", "śródręcze": "hand/palm", "srodrecze": "hand/palm"}
SEGMENT_ALIASES = {"hypothenar-eminence": "hypothenar", "thenar-eminence": "thenar", "central-palm-eminence": "central-palm"}
...
def _normalize_segment(value: Any) -> str:
    ...
    segment = re.sub(r"[^a-z0-9_-]+", "-", raw).strip("-")
    return SEGMENT_ALIASES.get(segment, segment)


def _canonical_spatial_id(value: Any) -> str:
    raw = str(value or "").strip().strip("/").lower()
    if raw in SPATIAL_ALIASES:
        return SPATIAL_ALIASES[raw]
    return "/".join(_normalize_segment(part) for part in raw.split("/") if _normalize_segment(part))


def _location(payload: dict[str, Any]) -> AnatomicalLocation:
    spatial_id = _canonical_spatial_id(payload.get("spatial_id") or "hand")
...
def _serialize(item: dict[str, Any]) -> dict[str, Any]:
    result = dict(item)
    result["spatial_id"] = _canonical_spatial_id(result.get("spatial_id") or "hand")
    return result
...
def list_observations(*, subject_id: str, timepoint: str | None = None, spatial_id: str | None = None, biological_level: str | None = None, include_archived: bool = False) -> list[dict[str, Any]]:
    ...
    selected_spatial_id = _canonical_spatial_id(spatial_id) if spatial_id else None
    for path in sorted(REGISTRY_ROOT.glob("*.json")):
        try:
            item = json.loads(path.read_text(encoding="utf-8"))
        except (OSError, json.JSONDecodeError):
            continue
        item_spatial_id = _canonical_spatial_id(item.get("spatial_id") or "hand")
        if item.get("subject_id") != subject_id: continue
        if timepoint and item.get("timepoint") != timepoint: continue
        if selected_spatial_id and item_spatial_id != selected_spatial_id: continue
        ...
        item["spatial_id"] = item_spatial_id
...
def get_observation(observation_id: str) -> dict[str, Any] | None:
    ...
    item = json.loads(path.read_text(encoding="utf-8"))
    item["spatial_id"] = _canonical_spatial_id(item.get("spatial_id") or "hand")
    return item
...
def create_observation(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    name = str(payload.get("name") or "").strip(); subject_id = str(payload.get("subject_id") or "").strip(); timepoint = str(payload.get("timepoint") or "").strip(); spatial_id = _canonical_spatial_id(payload.get("spatial_id"))
...
def update_observation(observation_id: str, patch: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if "biological_level" in patch:
        ...
        if level not in LEVELS: raise ValueError(f"biological_level must be one of: {', '.join(sorted(LEVELS))}")
    if "spatial_id" in patch:
        patch = {**patch, "spatial_id": _canonical_spatial_id(patch.get("spatial_id"))}
    ...
    allowed = {"name", "value", "observed_at", "source", "notes", "modality", "biological_level", "spatial_id", "evidence_id", "evidence_confidence", "evidence_type", "validated_interpretations", "source_measurement_ids"}
    ...
    item.update(changes); item["spatial_id"] = _canonical_spatial_id(item.get("spatial_id") or "hand"); item["author"] = author; item["version"] = previous_version + 1; item["updated_at"] = now
