...
def _location(payload: dict[str, Any]) -> AnatomicalLocation:
    ...
    return AnatomicalLocation(
        id=spatial_id,
        name=str(payload.get("location_name") or spatial_id.rsplit("/", 1)[-1]),
        level=str(payload.get("location_level") or "site"),
...
def _domain(payload: dict[str, Any], *, observation_id: str, version: int, created_at: str, updated_at: str) -> Observation:
    return Observation(
        ...
        timepoint_id=str(payload["timepoint"]),
        name=str(payload["name"]),
        value=payload.get("value"),
        observed_at=datetime.fromisoformat(str(payload["observed_at"]).replace("Z", "+00:00")),
        anatomical_location=_location(payload),
        source_measurement_ids=list(payload.get("source_measurement_ids") or []),
        metadata={
            "biological_level": payload["biological_level"],
...
def list_observations(*, subject_id: str, timepoint: str | None = None, spatial_id: str | None = None, biological_level: str | None = None, include_archived: bool = False) -> list[dict[str, Any]]:
    ...
    for path in sorted(REGISTRY_ROOT.glob("*.json")):
        ...
        if item.get("subject_id") != subject_id:
            continue
        if timepoint and item.get("timepoint") != timepoint:
            continue
        if spatial_id and item.get("spatial_id") != spatial_id:
            continue
        if biological_level and item.get("biological_level") != biological_level:
            continue
...
def create_observation(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if level not in LEVELS:
        raise ValueError(f"biological_level must be one of: {', '.join(sorted(LEVELS))}")
    name = str(payload.get("name") or "").strip()
    subject_id = str(payload.get("subject_id") or "").strip()
    timepoint = str(payload.get("timepoint") or "").strip()
    spatial_id = str(payload.get("spatial_id") or "").strip()
    if not subject_id or not timepoint or not spatial_id or not name:
        raise ValueError("subject_id, timepoint, spatial_id and name are required")
    ...
    item = {
        ...
        "location_name": payload.get("location_name") or spatial_id.rsplit("/", 1)[-1],
        "location_level": payload.get("location_level") or "site",
        "parent_id": payload.get("parent_id"),
        "biological_level": level,
        "modality": str(payload.get("modality") or "manual-entry"),
        "name": name,
        "value": payload.get("value"),
        "observed_at": payload.get("observed_at") or now,
        "source": str(payload.get("source") or "manual-entry").strip(),
        ...
        "created_at": now,
        "updated_at": now,
        "audit": [{"version": 1, "action": "created", "at": now, "source": str(payload.get("source") or "manual-entry")}],
    }
