...
def test_create_dataset_creates_manifest(tmp_path: Path, monkeypatch):
    ...
    created = dm.create_dataset(name="my cohort", modality="hand", description="Longitudinal hand cohort", source="own cohort", version="1.0", tags=["hand", "longitudinal"])

    assert created["dataset_id"].startswith("DS-")
    assert created["status"] == "draft"
    assert (tmp_path / created["root_path"]).exists()
...
def test_refresh_manifest_tracks_files(tmp_path: Path, monkeypatch):
    ...
    dataset_root.joinpath("subject-001", "T0").mkdir(parents=True)
    dataset_root.joinpath("subject-001", "T0", "front.jpg").write_bytes(b"image")
    dataset_root.joinpath("subject-001", "T0", "empty.jpg").write_bytes(b"")
