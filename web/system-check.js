(() => {
  ...
  async function runHealthCheck() {
    ...
    try {
      const [statusResponse, datasetsResponse] = await Promise.all([originalFetch('/api/status'), originalFetch('/api/datasets')]);
      ...
      healthFinished = true;
      if (!dashboardRequestsSeen) setStatus(`Backend ready · ${count} datasets · dashboard script did not start its checks`);
      else setStatus(`System ready · backend OK · ${count} datasets · ${seconds(performance.now() - started)}`);
      if (backend?.status && backend.status !== 'ready') setStatus(`Checking system · backend status: ${backend.status}`);
    } catch (error) {
      healthFinished = true;
      setStatus(`Checking system · backend check failed: ${error?.message || error}`);
    }
  }
