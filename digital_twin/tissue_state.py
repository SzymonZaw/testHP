...
This module stores tissue-level biological measurements and aggregates
precomputed cell populations without discarding cellular provenance.
"""
...
class TissueState:
    ...
    def aggregate_cells(self, cells: Iterable[CellProfile], confidence: Optional[float] = None) -> None:
        ...
        self.cellular_heterogeneity = 1.0 - max(self.health_distribution.values()) / self.cell_count if self.cell_count else 0.0
        inferred = sum(confidence_values) / len(confidence_values) if confidence_values else 0.0
        self.confidence = max(0.0, min(1.0, float(confidence if confidence is not None else inferred)))
        self.timestamp = datetime.utcnow().isoformat()

    def aggregate_populations(self, populations: Iterable[CellPopulation], confidence: Optional[float] = None) -> None:
        """Fold precomputed populations into tissue state and retain source cells."""
        population_list = tuple(populations)
        ...
        if len(source_ids) != len(set(source_ids)):
            raise ValueError("cell populations contain duplicate source cell ids")
        self.metadata["source_cell_ids"] = tuple(sorted(source_ids))
        ages = [population.biological_age_mean for population in population_list if population.biological_age_mean is not None]
        if ages:
            weights = [population.cell_count for population in population_list if population.biological_age_mean is not None]
            self.biological_age = sum(age * weight for age, weight in zip(ages, weights)) / sum(weights)
            ranges = [(population.biological_age_min, population.biological_age_max) for population in population_list if population.biological_age_min is not None and population.biological_age_max is not None]
            self.biological_age_range = (min(item[0] for item in ranges), max(item[1] for item in ranges)) if ranges else None
        else:
            self.biological_age = None
            self.biological_age_range = None
        inferred = sum(population.confidence * population.cell_count for population in population_list) / self.cell_count if self.cell_count else 0.0
        self.confidence = max(0.0, min(1.0, float(confidence if confidence is not None else inferred)))
        self.timestamp = datetime.utcnow().isoformat()
    ...
    def summary(self) -> Dict[str, Any]:
        return {
            ...
            "source_cell_count": len(self.metadata.get("source_cell_ids", ())),
