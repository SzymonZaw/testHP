...
(() => {
  ...
  async function collectRegistryDiagnostics(target = resolveCanonicalTarget()) {
    target = canonicalSpatialId(target) || 'hand';
    ...
    try {
      let response = await fetch(endpoint, { cache: 'no-store' }); diagnostics.status = response.status;
      ...
      // Older registry rows may still use the label-derived `...-eminence` id.
      // Canonical navigation must not lose those records while the registry is
      // being migrated. Query the legacy alias only when the canonical query is empty.
      ...
      diagnostics.matchDebug = { ...(diagnostics.matchDebug || {}), ...(payload.debug || {}), decisions, accepted: decisions.filter(d => d.matched), rejected: decisions.filter(d => !d.matched), rejectedCount: decisions.filter(d => !d.matched).length, target, aliasUsed };
  ...
  function resolveCanonicalTarget() {
    ...
    if (activeKey) {
      ...
      if (match?.[1]) return canonicalSpatialId(match[1].includes('/') ? match[1] : null);
    }
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => { syncCanonical(); window.spatialEvidenceTarget = resolveCanonicalTarget(); bootDebug(); }, { once: true }); else { syncCanonical(); window.spatialEvidenceTarget = resolveCanonicalTarget(); bootDebug(); }
