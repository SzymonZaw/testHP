...
(() => {
  ...
  const canonicalSpatialId = value => {
    ...
    const raw = typeof value === 'string'
      ? value
      : value.spatial_node_id || value.spatial_id || value.spatialId || value.targetSpatialId || value.target || value.spatialTarget || value.label || null;
  ...
  const registryCandidates = target => {
    const canonical = canonicalSpatialId(target);
    if (!canonical) return [];
    const aliases = [...CANONICAL_DEEP_IDS.entries()]
  ...
  const toUX = item => ({
    id: item.asset_id || item.evidence_id,
    evidenceId: item.evidence_id || '',
    backendAssetId: item.asset_id || '',
    type: item.spatial_level === 'tissue' ? 'Tissue' : item.spatial_level === 'cellular' || item.spatial_level === 'cell' ? 'Cellular' : item.modality === 'rna' ? 'Molecular' : 'Macro',
    sourceType: item.source === 'upload' ? 'upload' : 'dataset',
    target: canonicalSpatialId(item.spatial_node_id || item.spatial_id || item.target) || 'hand',
    spatial_id: canonicalSpatialId(item.spatial_node_id || item.spatial_id || item.target) || 'hand',
  ...
  const summarize = item => ({
    ...
    spatial_node_id: canonicalSpatialId(item.spatial_node_id) || null,
    spatial_id: canonicalSpatialId(item.spatial_id) || null,
    target: canonicalSpatialId(item.target) || null,
  ...
  const localDecision = (item, target) => {
    const actual = canonicalSpatialId(item.spatial_node_id);
    const expected = canonicalSpatialId(target);
    const matched = actual === expected;
    let reason = 'SPATIAL_ID_MISMATCH';
    if (matched) reason = 'EXACT_SPATIAL_ID_MATCH';
    else if (actual === 'hand' && String(expected).startsWith('hand/')) reason = 'ROOT_ONLY_REGISTERED_ASSET_NOT_DEEP_ATTACHED';
    else if (!actual) reason = 'MISSING_SPATIAL_NODE_ID';
    return { ...summarize(item), expected_spatial_node_id: expected, actual_spatial_node_id: actual, matched, reason };
  };
  ...
  function renderDiagnostics(d) {
    ...
    if (!panel) {
      ...
      panel.id = 'testhp-registry-debug-panel';
      panel.style.cssText = 'margin-top:14px;border:1px solid var(--border,#d8dee8);border-radius:12px;padding:14px;background:var(--panel,#fff);font-size:13px';
      studio.appendChild(panel);
    }
    const rejected = d.matchDebug?.rejected || [];
    const accepted = d.matchDebug?.accepted || [];
  ...
  async function collectRegistryDiagnostics(target = resolveCanonicalTarget()) {
    target = syncManagerTarget(target) || 'hand';
    window.spatialEvidenceTarget = target;
    const candidates = registryCandidates(target);
    const endpoint = `/api/spatial/registry?subject_id=own_cohort&timepoint=T0&spatial_node_id=${encodeURIComponent(String(target))}&debug=true`;
    ...
    try {
      ...
      if (!items.length && candidates.length > 1) {
        for (const candidate of candidates.slice(1)) {
          ...
          const aliasResponse = await fetch(aliasEndpoint, { cache: 'no-store' });
          if (!aliasResponse.ok) continue;
          const aliasPayload = await aliasResponse.json();
          const aliasItems = Array.isArray(aliasPayload.items) ? aliasPayload.items : [];
          if (aliasItems.length) {
            items = aliasItems;
            decisions = aliasItems.map(item => localDecision(item, target));
            aliasUsed = candidate;
            break;
          }
        }
      ...
      if (!payload.debug || !Array.isArray(payload.debug.decisions)) {
        ...
        if (fallbackResponse.ok) {
          ...
          diagnostics.matchDebug = { sourceWarning: 'SERVER DEBUG PAYLOAD UNAVAILABLE — decisions reconstructed locally from the unfiltered canonical registry.', fallbackEndpoint };
        } else {
  ...
  function resolveCanonicalTarget() {
    ...
    const activeSpatialTarget = canonicalSpatialId(active?.spatial_node_id || active?.spatial_id || active?.spatialId);
  ...
  // IMPORTANT: spatial-change events are high-frequency state notifications.
  // They must never trigger a diagnostic HTTP request. Diagnostics remain
  // available explicitly through window.__testhpCollectRegistryDiagnostics().
  let lastSpatialTarget = null;
  const onSpatialChange = event => {
    const detail = event?.detail || {};
    const target = syncManagerTarget(
      detail.spatial_id || detail.spatialId || detail.target?.spatial_id || detail.target?.spatialId || detail.target?.label || resolveCanonicalTarget()
    );
    if (!target || target === lastSpatialTarget) return;
    lastSpatialTarget = target;
    window.spatialEvidenceTarget = target;
    window.dispatchEvent(new CustomEvent('testhp:evidence-ux-refresh', { detail: { target, canonical: true } }));
  };
  window.addEventListener('testhp:spatial-layer-changed', onSpatialChange);
  window.addEventListener('testhp:spatial-target-changed', onSpatialChange);
  ...
  async function syncCanonical() {
    try {
      ...
      localStorage.setItem(STORAGE, JSON.stringify({
        evidence: [...canonicalUX, ...manual],
        target: canonicalSpatialId(current.target || resolveCanonicalTarget()) || 'hand'
      }));
  ...
  const boot = () => {
    syncManagerTarget(resolveCanonicalTarget());
    window.spatialEvidenceTarget = resolveCanonicalTarget();
    syncCanonical();
  };
