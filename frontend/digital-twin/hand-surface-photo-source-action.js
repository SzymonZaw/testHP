(() => {
  ...
  function addAction() {
    const sourceHead = [...document.querySelectorAll('#hand-surface-unified h3,strong,h2,.hsu-subnav button')].find(el => (el.textContent || '').trim() === 'Zdjęcia / źródła');
    ...
    action.innerHTML = `<div><strong>Dodaj materiał fotograficzny</strong><div style="font-size:12px;color:#667085;margin-top:3px">Zdjęcia zostaną przypisane do aktualnie wybranego celu: <code>${esc(targetId(t) || 'brak')}</code>.</div></div><button id="hand-photo-source-add-button" type="button" class="primary">＋ Dodaj zdjęcia</button>`;
