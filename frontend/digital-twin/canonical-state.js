...
export function createDigitalTwinState(initial = {}) {
  const selection = {
    ...
    region: initial.selection?.region ?? 'palm',
    tissue: initial.selection?.tissue ?? null,
    cell: initial.selection?.cell ?? null,
    molecularLayer: initial.selection?.molecularLayer ?? null,
  };
  return {
    ...
    biologicalState: initial.biologicalState ?? {
      health: notEstablished(),
      diseaseState: notEstablished(),
      biologicalAge: notEstablished(),
      confidence: null,
      uncertainty: null,
...
export function reduceAnalysisResult(state, backendResult) {
  ...
  next.input.subject_id = next.selection.subject;
  next.input.timepoint = next.selection.timepoint;
  next.qc = result.qc;
  ...
  next.anatomy = result.anatomy && typeof result.anatomy === 'object' ? result.anatomy : next.anatomy;
...
export function setSelection(state, patch = {}) {
  const next = createDigitalTwinState(state);
  next.selection = { ...next.selection, ...patch };
