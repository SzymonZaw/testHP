...
const normalizeSelection = (initial = {}) => ({
  subject: initial.subject ?? 'own_cohort',
  timepoint: initial.timepoint ?? 'T0',
  region: initial.region ?? 'palm',
  tissue: initial.tissue ?? null,
  cell: initial.cell ?? null,
  molecularLayer: initial.molecularLayer ?? null,
});
...
export function createDigitalTwinState(initial = {}) {
  ...
  return {
    ...
    // Canonical selection. Components should read these fields from the shared state.
    subject: selection.subject,
    timepoint: selection.timepoint,
    region: selection.region,
    tissue: selection.tissue,
    cell: selection.cell,
    molecularLayer: selection.molecularLayer,

    // Compatibility mirror for older presentation modules. Mutations must go through setSelection.
    selection,

    input: { input_id: null, modalities: {}, ...initial.input },
    ...
    cells: Array.isArray(initial.cells) ? initial.cells : [],
    trajectory: initial.trajectory ?? null,
    diseaseTrajectory: initial.diseaseTrajectory ?? null,
    whatIf: initial.whatIf ?? null,
    ...
    biologicalState: initial.biologicalState ?? {
      health: emptyResult(),
      diseaseState: emptyResult(),
      biologicalAge: emptyResult(),
      confidence: null,
      uncertainty: null,
      status: NOT_ESTABLISHED,
    },
    ...
    modelMetadata: initial.modelMetadata ?? null,
  };
}

function syncCanonicalSelection(state) {
  const selection = normalizeSelection({
    ...
    tissue: state.tissue,
    cell: state.cell,
    molecularLayer: state.molecularLayer,
  });
...
export function reduceAnalysisResult(state, backendResult) {
  ...
  next.input.subject_id = next.subject;
  next.input.timepoint = next.timepoint;
  next.qc = result.qc;
  next.biologicalAge = result.biological_age;
  next.health = result.health_state;
  next.molecular = { ...next.molecular, states: result.molecular_states };
  next.provenance = result.provenance;
  next.validation = result.validation;
  next.modelMetadata = result.model_metadata;
  next.uncertainty = result.multimodal_state?.uncertainty ?? null;
  next.interventions = result.intervention_priority;
  next.whatIf = result.what_if;
  next.assets = result.assets;
  next.cells = result.cells;
  next.trajectory = result.trajectory;
  next.diseaseTrajectory = result.disease_trajectory;
  next.evidence = {
    ...next.evidence,
    coverage: result.evidence?.coverage ?? null,
    ...
    items: Array.isArray(result.evidence?.items) ? result.evidence.items : (result.provenance ? [result.provenance] : []),
    validation: result.validation,
  };
  next.biologicalState = {
    health: result.health_state ?? emptyResult(),
  ...
  next.anatomy = {
    hand: anatomy.hand ?? next.anatomy.hand,
    regions: Array.isArray(anatomy.regions) ? anatomy.regions : next.anatomy.regions,
  ...
  return syncCanonicalSelection(next);
}
...
export function setSelection(state, patch = {}) {
  ...
  const selection = normalizeSelection({
    ...
    timepoint: patch.timepoint ?? current.timepoint,
    region: patch.region ?? current.region,
    tissue: patch.tissue ?? current.tissue,
    cell: patch.cell ?? current.cell,
    molecularLayer: patch.molecularLayer ?? current.molecularLayer,
  });

  if ('region' in patch) {
    selection.tissue = null;
    selection.cell = null;
    selection.molecularLayer = null;
  }
  if ('tissue' in patch) {
    selection.cell = null;
    selection.molecularLayer = null;
  }
  if ('cell' in patch && !patch.cell) selection.molecularLayer = null;
