(() => {
  ...
  const RANGES = {
    palmLength: [0.75, 1.25],
    palmWidth: [0.75, 1.25],
    thickness: [0.75, 1.25],
    fingerSpread: [0.7, 1.3],
    taper: [0.7, 1.3],
    thumbAngle: [0.7, 1.3],
  };
  ...
  const applyToMain = () => {
    ...
    palm.scale.set(
      pb.scale.x * state.palmWidth,
      pb.scale.y * state.palmLength,
      pb.scale.z * state.thickness,
    );
    ...
    if (thumb) {
      ...
      thumb.scale.set(
        b.scale.x * (1 - 0.10 * (state.taper - 1)),
        b.scale.y,
        b.scale.z * state.thickness,
      );
    }

    window.dispatchEvent(new CustomEvent('testhp:hand-surface-geometry-changed', {
      detail: { parameters: { ...state }, meshCount: meshes.size, source: 'digitalTwinGeometry' },
    }));
    return meshes;
  };

  const save = () => localStorage.setItem(KEY, JSON.stringify({
    schema: 'hand-surface-geometry-live-v1',
    parameters: state,
  ...
  const geometryApi = () => {
    ...
    bridge.setState = next => {
      Object.keys(PARAMS).forEach(name => {
        if (next?.[name] != null) bridge.setParameter(name, next[name]);
      });
  ...
  const geometryRoot = () => {
    const title = [...document.querySelectorAll('strong')]
      .find(el => el.textContent?.trim() === 'Geometria dłoni');
  ...
  const setStatus = text => {
    const status = previewHost?.querySelector('[data-geometry-preview-status]');
    if (status) status.textContent = text;
  };

  const updateMainConnection = () => {
    const status = previewHost?.querySelector('[data-geometry-main-status]');
    if (!status) return;
  ...
  const updatePreview = () => {
    ...
    const palm = previewMeshes.get('palm');
    if (palm) palm.scale.set(state.palmWidth, state.palmLength, state.thickness);

    const baseX = [-1.05, -0.35, 0.42, 1.12];
    FINGERS.forEach((name, index) => {
      const mesh = previewMeshes.get(name);
      if (!mesh) return;
      const center = (FINGERS.length - 1) / 2;
      const spreadOffset = (index - center) * 0.20 * (state.fingerSpread - 1);
      const width = 1 - 0.22 * (state.taper - 1);
      mesh.position.x = baseX[index] + spreadOffset;
      mesh.scale.set(width, 1, state.thickness);
    });

    const thumb = previewMeshes.get('thumb');
    if (thumb) {
      thumb.rotation.z = -0.82 - 0.42 * (state.thumbAngle - 1);
      thumb.scale.set(1 - 0.10 * (state.taper - 1), 1, state.thickness);
    }

    setStatus(Object.values(state).every(value => Number(value) === 1)
      ? 'Live · wartości domyślne'
      : 'Live · zmieniona geometria');
  };
  ...
  const makePreview = async () => {
    ...
    try {
      ...
      const add = (id, position, radius, length, rotation = [0, 0, 0]) => {
        ...
        root.add(mesh);
        previewMeshes.set(id, mesh);
      };

      add('wrist', [0, -2.15, 0], 0.72, 1.25);
      add('palm', [0, -0.35, 0], 1.55, 2.25);
      add('thumb', [-1.45, 0, 0.02], 0.48, 1.45, [0, 0, -0.82]);
      add('index', [-1.05, 1.95, 0], 0.43, 2.15);
      add('middle', [-0.35, 2.25, 0], 0.46, 2.55);
      add('ring', [0.42, 2.12, 0], 0.45, 2.32);
      add('little', [1.12, 1.86, 0], 0.40, 1.95, [0, 0, 0.08]);
      ...
      setStatus(`Podgląd niedostępny: ${error.message}`);
    }
  };
  ...
  const bindSliders = root => {
    ...
    const map = [
      ['palmLength', /długość dłoni/i],
      ['palmWidth', /szerokość dłoni/i],
      ['thickness', /grubość powierzchni/i],
      ['fingerSpread', /rozstaw palców/i],
      ['taper', /zwężenie palców/i],
      ['thumbAngle', /ustawienie kciuka/i],
    ];
    inputs.forEach(input => {
      if (input.dataset.geometryLiveBound === '1') return;
      const text = input.closest('label,div')?.textContent || '';
      const match = map.find(([, regex]) => regex.test(text));
      if (!match) return;
      input.dataset.geometryLiveBound = '1';
      input.addEventListener('input', () => window.digitalTwinGeometry.setParameter(match[0], input.value));
    });
  };
  ...
  const syncSliders = root => {
    ...
    [...root.querySelectorAll('input[type="range"]')].forEach(input => {
      const text = input.closest('label,div')?.textContent || '';
      const match = map.find(([, regex]) => regex.test(text));
      if (match) input.value = state[match[0]];
    });
  };
  ...
  const installUi = () => {
    ...
    host.innerHTML = `
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:flex-start;padding:13px 14px;border-bottom:1px solid var(--border,#d8dee8)">
        <div><strong style="display:block;font-size:14px">Podgląd 3D</strong><span style="display:block;margin-top:3px;font-size:12px;color:#667085">Przesuwaj suwaki i obserwuj dokładnie ten sam efekt w podglądzie.</span></div>
        <span data-geometry-preview-status style="font-size:11px;font-weight:800;color:#027a48">Uruchamianie…</span>
      </div>
      <div data-geometry-preview-canvas style="height:360px;background:#0d1117;position:relative">
    ...
    previewHost = host.querySelector('[data-geometry-preview-canvas]');
    previewCanvas = host.querySelector('canvas');

    // Never use insertBefore with a node that may have been moved by another UI owner.
    // The geometry intro is stable and its parent is the actual geometry content container.
    if (root.intro && root.intro.parentElement === root.container) {
      root.container.insertBefore(host, root.intro.nextSibling);
    } else {
      root.container.appendChild(host);
    }

    bindSliders(root.container);
    syncSliders(root.container);
    ...
    observer.observe(root.container, { childList: true, subtree: true });
    return true;
  };
  ...
  const updateUi = () => {
    ...
    if (status) status.textContent = Object.values(state).every(value => Number(value) === 1)
      ? 'Live · wartości domyślne'
      : 'Live · zmieniona geometria';
    updateMainConnection();
  };
  ...
  const boot = () => {
    ...
    if (!bootObserver) {
      bootObserver = new MutationObserver(() => installUi() && bootObserver.disconnect());
