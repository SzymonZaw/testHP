  ...
  const boot = () => {
    ...
    const refreshEvidenceDiagnostics=async detail=>{
      ...
      try {
        ...
        const st=await sr.text(), ot=await or.text(); let sp=null,op=null;
        try{sp=JSON.parse(st);}catch(_){} try{op=JSON.parse(ot);}catch(_){}
        if(seq!==stateRequestSeq)return;
        lastStatePayload=sp; lastObservationsPayload=op;
