(() => {
  const boot = () => {
    ...
    Object.assign(host.style, {
      ...
      zIndex: '2147483647', width: 'min(720px, calc(100vw - 32px))',
      maxWidth: 'min(720px, calc(100vw - 32px))', pointerEvents: 'auto', display: 'block'
    });
    // Always move the overlay outside the WebGL viewport. This prevents any
    // transform/overflow/stacking context on the viewport from hiding it.
    ...
    const writeLog = message => { lines.push(`[${now()}] ${message}`); while (lines.length > 200) lines.shift(); log.textContent = lines.join('\n'); log.scrollTop = log.scrollHeight; };
    const spatial = () => {
    ...
    const rendererInfo = () => {
      ...
      let three = 'module-scoped / not global'; try { if (window.THREE?.REVISION) three = window.THREE.REVISION; } catch (_) {}
