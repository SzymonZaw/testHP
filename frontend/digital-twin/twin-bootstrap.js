(() => {
  ...
  function ensureLegacyMacroPreviewCompatibility() {
    ...
    const row = document.querySelector('.macro-row');
    if (!row) return;
    const preview = document.createElement('div');
    preview.id = 'macro-preview';
    preview.className = 'evidence-preview';
    preview.hidden = true;
    preview.style.setProperty('display', 'none', 'important');
    preview.innerHTML = '<img id="macro-image" alt=""><div class="preview-caption"><span id="macro-filename"></span><span id="macro-view"></span></div>';
  ...
  function collectBootDiagnostics(error = bootState.failure) {
    ...
    const breadcrumb = [...document.querySelectorAll('#spatial-breadcrumb button')].map(el => el.textContent.trim()).filter(Boolean);
    const children = [...document.querySelectorAll('#spatial-children .spatial-target')].map(el => ({
      label: el.querySelector('strong')?.textContent?.trim() || '(brak etykiety)',
      id: el.dataset?.spatialId || el.getAttribute('data-spatial-id') || null,
      disabled: !!el.disabled,
      connected: el.isConnected
    }));
    const rect = canvas?.getBoundingClientRect?.();
    let webgl = 'nie sprawdzono';
    if (canvas) {
      try {
        const gl = canvas.getContext('webgl2') || canvas.getContext('webgl');
        webgl = gl ? `${gl.constructor?.name || 'WebGL'} OK` : 'BRAK kontekstu WebGL/WebGL2';
      } catch (webglError) {
        webgl = `BŁĄD getContext(): ${webglError.message}`;
      }
    }
    const scripts = [...document.scripts]
      .map(script => script.src)
      .filter(Boolean)
      .filter(src => src.includes('/digital-twin/'));
    const diagnostics = {
      failure: error ? {
      ...
      boot: {
        currentStep: bootState.currentStep,
        completedSteps: bootState.completedSteps,
        initAgeMs: Math.round(performance.now() - bootState.startedAt),
        lastProgress: bootState.lastProgress,
        recentProgress: bootState.recentProgress
      },
      dom: {
        twinViewport: !!viewport,
        twinCanvas: !!canvas,
        viewerLoading: !!loading,
        debugHost: !!document.getElementById('twin-viewport-debug-host'),
        spatialNode: !!node,
        spatialChildren: !!document.getElementById('spatial-children'),
        connected: {
          viewport: !!viewport?.isConnected,
      ...
      renderer: {
        ...
        activeLayer: manager?.activeLayer || manager?.active?.activeLayer || null,
        activeType: manager?.active?.constructor?.name || null,
        ...
        canonical: !!(manager && manager.version === 'canonical-three-1' && manager.active?.scene && manager.active?.camera && manager.deepRenderer)
      },
      environment: {
        readyFlag: !!window.__testhpTwinReady,
        bootComplete: !!window.__testhpTwinBootComplete,
        three: typeof window.THREE !== 'undefined' ? 'global THREE present' : 'global THREE not exposed',
        url: location.href,
        userAgent: navigator.userAgent,
        loadedDigitalTwinScripts: scripts
      }
    };
    window.__testhpTwinBootDiagnostics = diagnostics;
    return diagnostics;
  }
  ...
  function renderBootDiagnostics(error) {
    ...
    if (!box) return;
    const diagnostics = collectBootDiagnostics(error);
    const existing = document.getElementById('twin-boot-failure-details');
    if (existing) existing.remove();
    const panel = document.createElement('div');
    panel.id = 'twin-boot-failure-details';
    panel.innerHTML = `
      <div class="twin-boot-failure-title">SZCZEGÓŁY AWARII</div>
  ...
  function showBootUi() {
    ...
    style.textContent = `
      #twin-boot-diagnostics{position:absolute;inset:16px auto auto 16px;z-index:50;width:min(720px,calc(100% - 32px));max-height:calc(100% - 32px);overflow:auto;padding:14px 16px;border:1px solid rgba(130,145,165,.35);border-radius:12px;background:rgba(13,17,23,.95);color:#e6edf3;font:12px/1.45 ui-monospace,SFMono-Regular,Menlo,monospace;box-shadow:0 12px 40px rgba(0,0,0,.25)}
      #twin-boot-diagnostics.twin-boot-minimized{width:auto;padding:0;border:0;background:transparent;box-shadow:none;overflow:visible}
      #twin-boot-diagnostics h3{margin:0;font:700 13px/1.2 system-ui,sans-serif;letter-spacing:.06em;text-transform:uppercase}
      #twin-boot-diagnostics.twin-boot-minimized h3{display:none!important}
      #twin-boot-diagnostics .twin-boot-heading{display:flex;align-items:center}
      ...
      #twin-boot-diagnostics.twin-boot-minimized .twin-boot-toggle{margin-left:0;padding:7px 10px}
      .twin-boot-line{display:grid;grid-template-columns:18px 190px 1fr;gap:6px;align-items:start;padding:3px 0;color:#9da7b3}
      .twin-boot-line strong{color:#d8dee4}.twin-boot-line.ok .twin-boot-mark{color:#56d364}.twin-boot-line.ok strong{color:#e6edf3}.twin-boot-line.error .twin-boot-mark,.twin-boot-line.error strong{color:#ff7b72}
      .twin-boot-summary{margin-top:8px;color:#8b949e}
      .twin-boot-failure-title{margin-top:12px;padding-top:10px;border-top:1px solid rgba(130,145,165,.25);font:700 11px/1.2 system-ui,sans-serif;letter-spacing:.08em;color:#ff7b72}
      ...
      #twin-boot-diagnostics.twin-boot-minimized > #twin-boot-lines,#twin-boot-diagnostics.twin-boot-minimized > .twin-boot-summary,#twin-boot-diagnostics.twin-boot-minimized > #twin-boot-failure-details{display:none!important}
      #twin-boot-diagnostics.twin-boot-minimized > *:not(.twin-boot-heading){display:none!important}
      #twin-boot-diagnostics.twin-boot-minimized .twin-boot-heading h3{display:none!important}
    `;
  ...
  window.addEventListener('error', event => {
    if (event?.message) pushProgress('WINDOW ERROR', `${event.message} | ${event.filename || ''}:${event.lineno || ''}:${event.colno || ''}`);
  });
  window.addEventListener('unhandledrejection', event => {
    pushProgress('UNHANDLED PROMISE', String(event.reason?.stack || event.reason || 'unknown'));
  });
  ...
  async function boot() {
    ...
    try {
      ...
      ensureLegacyMacroPreviewCompatibility();

      log('Three.js + canonical viewport', 'import app.js');
      await withTimeout(import('./app.js?v=canonical-spatial-3d-27'), 15000, 'Kanoniczny widok');
      mark('Three.js + canonical viewport', true, 'app.js załadowano');

      await loadClassic('/digital-twin/spatial-layer-viewport.js?v=canonical-13', 'Spatial bridge');
      await loadClassic('/digital-twin/deep-viewport-sync.js?v=canonical-deep-3', 'Deep 3D sync');
      ...
      await loadClassic('/digital-twin/stages-2-4.js?v=stage-2-4-10', 'Spatial stages 2–4');
      await loadClassic('/digital-twin/biological-state-ui.js?v=biological-state-3', 'Biological state UI');
      loadStages58NonBlocking();
      await loadClassic('/digital-twin/evidence-ux.js?v=evidence-ux-7', 'Evidence UX');
      log('Viewport boot verifier', 'uruchamianie weryfikatora');
      await withTimeout(import('./twin-viewport-boot.js?v=boot-5'), 10000, 'Weryfikator uruchomienia widoku');
      ...
      button.onclick = async () => {
        ...
        try {
          ...
          mark('Photo reconstruction', false, error.message);
          console.error(error);
        }
      };
      ...
      pushProgress('BOOT COMPLETE', 'główna ścieżka uruchomiona; etapy 5–8 i rekonstrukcja zdjęciowa nie blokują startu');
    } catch (error) {
      bootState.failure = error;
      console.error('[Twin Bootstrap]', error);
      const current = [...document.querySelectorAll('#twin-boot-lines .twin-boot-line')]
        .reverse()
        .find(x => !x.classList.contains('ok'))
        ?.querySelector('strong')?.textContent;
      if (current) mark(current, false, error.message);
      pushProgress('BOOT FAILURE', `${error.name || 'Error'}: ${error.message}`);
      renderBootDiagnostics(error);
      const loading = document.getElementById('viewer-loading');
      if (loading) {
        loading.hidden = false;
        loading.style.display = 'grid';
        loading.textContent = `Uruchomienie cyfrowego bliźniaka nie powiodło się: ${error.message}`;
        loading.classList.add('viewer-loading-error');
      }
      window.dispatchEvent(new CustomEvent('testhp:twin-error', { detail: { error, diagnostics: collectBootDiagnostics(error) } }));
    }
  }
