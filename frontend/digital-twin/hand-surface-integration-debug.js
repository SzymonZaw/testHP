(() => {
  ...
  function ensureEvidenceDebugHelper() {
    if (window.testhpEvidenceTargetDebug || document.querySelector(`script[src^="${EVIDENCE_DEBUG_SCRIPT.split('?')[0]}"]`)) return;
    const script = document.createElement('script');
    script.src = EVIDENCE_DEBUG_SCRIPT;
    script.onload = () => window.dispatchEvent(new CustomEvent('testhp:evidence-target-debug-ready'));
  ...
  function photoConsistency(t) {
    ...
    return { id, evidence, assigned, prepared, registered, duplicateViews, targetMappings, planReady, planTarget, planViews, packageState, reconstructionKnown, reconstructionReady, applied, canonicalAccepted, canonicalRejected, canonicalExpected, canonicalTargetConsistent, viewSetConsistent, registrationConsistent, verdict: canonicalTargetConsistent && viewSetConsistent && registrationConsistent && duplicateViews.length === 0 };
  }
  ...
  function installDatasetWriterTrace() {
    ...
    const observer = new MutationObserver(mutations => mutations.forEach(m => {
      if (m.type === 'attributes' && m.target === document.body && m.attributeName === 'data-spatial-target') push({type:'body.dataset mutation', attribute:m.attributeName, previous:m.oldValue, next:document.body.dataset.spatialTarget || null, targetId:targetId(target())});
    }));
  ...
  function evidenceTargetDebugBlock(t) {
    ...
    const sampleLines = result.samples.map(x => `  [${x.index}] linked=${x.linked?'YES':'NO'} spatial_id=${x.spatial_id ?? 'NULL'} spatialId=${x.spatialId ?? 'NULL'} target=${x.target ?? 'NULL'} region=${x.region ?? 'NULL'} canonical=${x.canonicalCandidates.join(', ') || 'NULL'}`).join('\n');
    return [
      'EVIDENCE TARGET DEBUG',
      `current raw target: ${result.currentTarget.raw ?? 'NULL'}`,
      `current canonical target: ${result.currentTarget.canonical ?? 'NULL'}`,
      `total UX-cache records: ${result.total}`,
      `target-linked by diagnostic matcher: ${result.linkedCount}`,
      `match strategy: ${result.matchStrategy}`,
      'samples:',
      sampleLines || '  (none)'
    ].join('\n');
  }
