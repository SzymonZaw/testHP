(() => {
  ...
  function importOwnHand() {
    ...
    else window.dispatchEvent(new CustomEvent('testhp:import-hand-requested'));
  }
  ...
  function startReference() {
    window.dispatchEvent(new CustomEvent('testhp:reference-hand-requested', {
      detail: { sourceId: 'nih-hand-template-3dpx-017237', regionId: 'palm' }
    }));
    choosePalm();
  }
  ...
  function mount() {
    ...
    // Use one delegated handler on the backdrop. This remains reliable even if
    // another frontend module installs document-level click handlers.
    backdrop.addEventListener('click', (event) => {
      event.stopPropagation();
      const action = event.target.closest('button[data-start-reference],button[data-start-import],button[data-skip]');
      if (!action) {
      ...
      if (action.hasAttribute('data-start-reference')) {
        close(backdrop);
        startReference();
      } else if (action.hasAttribute('data-start-import')) {
        close(backdrop);
        importOwnHand();
      } else {
        close(backdrop);
  ...
  function boot() {
    ...
    mount();
    const timer = setInterval(() => mount(), 250);
