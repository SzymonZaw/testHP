...
function applySpatialNode(){
  ...
  window.dispatchEvent(new CustomEvent('testhp:spatial-layer-changed',{detail:{level:current.level,target:current.label,path:spatialPath.map(x=>x.label),children:childTargets(current).map(x=>x.label)}}));
}
...
canvas.addEventListener('click',e=>{
  ...
  if(current.level!=='macro'){
    window.dispatchEvent(new CustomEvent('testhp:canonical-macro-click-skipped',{detail:{reason:'deep-spatial-target-active',level:current.level,target:current.label,path:spatialPath.map(x=>x.label)}}));
    return;
  }
  ...
  const hits=ray.intersectObjects([...regionMeshes.values()],false);
...
function publishViewportManager(){
  window.spatialViewportManager={
    ...
    render(){
      ...
      resize();
      const postRender=window.testhpViewportPostRender;
