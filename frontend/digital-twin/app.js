...
function updateInspectorForSpatial(node){
  const region=node.regionId||selectedRegion;
  const s=status(region);
  setText('zone-label',region);setText('region-title',node.label||REGION_LABELS[region]||region);setText('region-context',`Hand · ${timepoint} · ${SCALE_LABELS[node.level]||node.level}`);
  const macroVisible=node.level==='macro';
  ...
  if(macroVisible){
    ...
    setEvidenceStatus('macro-status',s.macro?'OBSERVED':'NONE');
    renderMacro(region);
    preview.closest('.evidence-row').hidden=false;
  }else{
    preview.hidden=true;preview.closest('.evidence-row').hidden=false;
    setText('macro-state','Not shown at this resolution');setText('macro-detail','Macro evidence remains attached to the parent spatial region; it is not presented as evidence for this deeper target.');setEvidenceStatus('macro-status','PARENT');
    const nav=document.getElementById('macro-gallery');if(nav)nav.remove();
  }
  const tissueHere=node.level==='tissue'?s.tissue:false;
  const cellularHere=node.level==='cellular'?s.cellular:false;
  const molecularHere=s.molecular;
  setText('tissue-state',node.level==='tissue'?(tissueHere?`${s.tissueCount} linked item${s.tissueCount===1?'':'s'}`:'No evidence at this resolution'):(node.level==='macro'?(s.tissue?`${s.tissueCount} linked item${s.tissueCount===1?'':'s'}`:'Unavailable'):'Parent evidence only'));
  setText('tissue-detail',node.level==='tissue'?(tissueHere?'Tissue / WSI evidence is explicitly linked to this spatial target.':'No tissue / WSI evidence is linked to this target. The node remains navigation only.'):(s.tissue?'Tissue evidence exists on this region but is not automatically inherited by this target.':'No tissue / WSI evidence is explicitly linked to this region.'));
  setEvidenceStatus('tissue-status',node.level==='tissue'?(tissueHere?'LINKED':'NONE'):(s.tissue?'REGION':'NONE'));
  setText('cellular-state',node.level==='cellular'?(cellularHere?`${s.cellularCount} linked item${s.cellularCount===1?'':'s'}`:'No evidence at this resolution'):(s.cellular?'Linked evidence':'Unavailable'));
  setText('cellular-detail',node.level==='cellular'?(cellularHere?'Cellular microscopy evidence is explicitly linked to this field.':'No cellular evidence is linked to this field. The visualization is a navigation target only.'):(s.cellular?'Cellular evidence exists but is not automatically inherited by this node.':'Cellular evidence requires explicitly linked microscopy data.'));
  setEvidenceStatus('cellular-status',node.level==='cellular'?(cellularHere?'LINKED':'NONE'):(s.cellular?'REGION':'NONE'));
  setText('molecular-state',molecularHere?`${s.molecularCount} linked item${s.molecularCount===1?'':'s'}`:'Unavailable');setText('molecular-detail',molecularHere?'Molecular measurements are explicitly linked to this region.':'No molecular measurements are explicitly linked to this region.');setEvidenceStatus('molecular-status',molecularHere?'LINKED':'NONE');
  const evidenceAtLevel=node.level==='macro'?s.macro:node.level==='tissue'?s.tissue:node.level==='cellular'?s.cellular:false;
  setText('confidence-state',evidenceAtLevel?'Observed evidence':'Navigation only');
  setText('evidence-level',evidenceAtLevel?`Evidence linked at ${SCALE_LABELS[node.level]}`:`No evidence at ${SCALE_LABELS[node.level]}`);
}
