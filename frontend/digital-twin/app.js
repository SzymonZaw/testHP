...
function renderMacroEvidence(region){
  const items=regionEvidence(region),preview=document.getElementById('macro-preview'),image=document.getElementById('macro-image'),filename=document.getElementById('macro-filename'),view=document.getElementById('macro-view');
...
function renderMacroGallery(region){
  const items=regionEvidence(region),preview=document.getElementById('macro-preview'); if(!preview)return;
  ...
  nav.replaceChildren();
  items.forEach((item,index)=>{const button=document.createElement('button');button.type='button';button.textContent=(item.view||item.filename||`View ${index+1}`).replaceAll('_',' ');button.className=index===selectedMacroIndex?'active':'';button.onclick=()=>{selectedMacroIndex=index;renderMacroEvidence(region);renderMacroGallery(region);};nav.appendChild(button);});
}
...
function selectRegion(region,focus=true){
  ...
  setText('zone-label',region);setText('region-title',REGION_LABELS[region]||region);setValue('form-region',region);
  ...
  setText('tissue-detail',s.tissue?'Tissue-level evidence is explicitly linked to this region.':'No tissue / WSI evidence is explicitly linked to this region.');setEvidenceStatus('tissue-status',s.tissue?'LINKED':'NONE');
  setText('cellular-state',s.cellular?`${s.cellularCount} linked item${s.cellularCount===1?'':'s'}`:'Unavailable');
  setText('cellular-detail',s.cellular?'Cellular evidence is explicitly linked to this region.':'Cellular conclusions require microscopy / cellular data explicitly linked to this region.');setEvidenceStatus('cellular-status',s.cellular?'LINKED':'NONE');
  setText('molecular-state',s.molecular?`${s.molecularCount} linked item${s.molecularCount===1?'':'s'}`:'Unavailable');
  setText('molecular-detail',s.molecular?'Molecular measurements are explicitly linked to this region.':'No molecular measurements are explicitly linked to this region.');setEvidenceStatus('molecular-status',s.molecular?'LINKED':'NONE');
  const availableLayers=[s.macro,s.tissue,s.cellular,s.molecular].filter(Boolean).length;
  setText('confidence-state',availableLayers?'Observed evidence':'No evidence');setText('evidence-level',availableLayers?`${availableLayers}/4 evidence layers available`:'Availability only');
  if(focus){controls.target.set(0,region==='wrist'?-1.2:region==='palm'?-0.35:1.3,0);controls.update();}
}
function updateGlobalCoverage(){
  const assets=availableAssets(),hand=assets.filter(x=>x.modality==='hand'),tissue=assets.filter(x=>x.modality==='wsi'),cellular=assets.filter(x=>['microscopy','cellular'].includes(String(x.modality).toLowerCase())),molecular=assets.filter(x=>x.modality==='rna');
  setText('coverage-macro',`Macro ${hand.length?100:0}%`);setText('coverage-micro',`Tissue ${tissue.length?100:0}%`);setText('coverage-cellular',`Cellular ${cellular.length?100:0}%`);setText('coverage-molecular',`Molecular ${molecular.length?100:0}%`);
}
async function refresh(){
  try{const r=await fetch(`/api/hand/analysis?subject_id=${encodeURIComponent(subjectId)}&timepoint=${encodeURIComponent(timepoint)}`);if(!r.ok)throw Error('Evidence request failed');analysis=await r.json();updateGlobalCoverage();setText('twin-status',`${handAssets().length} macro observation${handAssets().length===1?'':'s'} loaded`);selectRegion(selectedRegion,false);}
...
document.getElementById('deep-analysis').onclick=()=>{const s=status(selectedRegion),layers=[s.macro&&'Macro',s.tissue&&'Tissue',s.cellular&&'Cellular',s.molecular&&'Molecular'].filter(Boolean);const message=layers.length?`Deep analysis · ${REGION_LABELS[selectedRegion]||selectedRegion}\n\nAvailable evidence layers: ${layers.join(', ')}.\n\nNo biological inference is generated automatically. Evidence must remain explicitly linked to this region.`:`Deep analysis · ${REGION_LABELS[selectedRegion]||selectedRegion}\n\nNo linked evidence is currently available. The system will not invent tissue, cellular or molecular findings.`;alert(message);};
document.getElementById('add-observation').onclick=()=>dialog.showModal();document.querySelector('.close').onclick=()=>dialog.close();
