(() => {
  ...
  const VIEWER_VERSION = 'reference-glb-safe-24';
  ...
  function card(m){
    ...
    c.innerHTML='<model-viewer class="dt-reference-3d-model" alt="NIH 3D healthy adult human hand reference template" loading="eager" reveal="auto" camera-controls touch-action="pan-y" interaction-prompt="none" shadow-intensity="0.05" exposure="0.95" camera-orbit="0deg 75deg 105%"></model-viewer><div class="dt-reference-3d-overlay"><div class="dt-reference-3d-title">REFERENCE HAND · NIH 3D · 3DPX-017237</div><div class="dt-reference-3d-source">GLB · PUBLIC REFERENCE</div><div class="dt-reference-3d-status">Loading NIH 3D reference geometry…</div><div class="dt-reference-3d-mapping">Region geometry mapping · NOT ESTABLISHED</div></div>';
  ...
  function fallback(c,msg){
    const viewer=c?.querySelector('.dt-reference-3d-model');if(!viewer)return;
    const st=c.querySelector('.dt-reference-3d-status');if(st)st.textContent=msg;
    viewer.removeAttribute('src');
    if(!c.querySelector('.dt-reference-3d-fallback')){
      const f=document.createElement('div');f.className='dt-reference-3d-fallback';
      f.innerHTML='<div><strong>Reference 3D viewer unavailable</strong><br><a href="'+NIH_ENTRY_URL+'" target="_blank" rel="noopener noreferrer">Open NIH 3D reference on NIH 3D</a></div>';c.appendChild(f);
    }
  }
  function markLoaded(token,model,c){
    if(token!==bootToken||!model||!model.loaded)return;
    stopTimers();
    activeModel=model;
    state({active:true,loading:false,loaded:true,error:null,loadMethod:'same_origin_fastapi'});
    const st=c.querySelector('.dt-reference-3d-status');if(st)st.textContent='Loaded NIH GLB · public reference geometry · not user health data';
  }
  async function load(token,model,c){
    try{
      ...
      if(!ct.includes('model/gltf-binary')&&!ct.includes('application/octet-stream'))throw new Error(`Reference GLB endpoint returned ${ct||'unknown content type'}`);
      const buf=new Uint8Array(await r.arrayBuffer());
      if(buf.length<4||String.fromCharCode(...buf.slice(0,4))!=='glTF')throw new Error('Reference asset is not a valid GLB');
      if(token!==bootToken)return;
      const onLoad=()=>markLoaded(token,model,c);
      const onError=()=>{if(token!==bootToken)return;stopTimers();state({active:true,loading:false,loaded:false,error:'GLB returned by FastAPI endpoint could not be decoded',loadMethod:'same_origin_fastapi'});fallback(c,'The verified NIH GLB was fetched but could not be decoded by model-viewer.');};
      ...
      if(model.loaded){markLoaded(token,model,c);return;}
      let checks=0;
      loadPollTimer=setInterval(()=>{checks++;if(model.loaded)markLoaded(token,model,c);else if(checks>=120){stopTimers();if(token===bootToken){state({active:true,loading:false,loaded:false,error:'Timed out waiting for model-viewer to finish loading',loadMethod:'same_origin_fastapi'});fallback(c,'The verified NIH GLB did not finish loading in the allotted time.');}}},250);
    }catch(e){
      if(token!==bootToken)return;
      stopTimers();state({active:true,loading:false,loaded:false,error:e?.message||'Reference GLB endpoint failed',loadMethod:'same_origin_fastapi'});fallback(c,e?.message||'The same-origin reference asset endpoint could not load the verified NIH GLB.');
    }finally{loadingPromise=null;}
  }
  function mount(token){
    const m=mountPoint();if(!m)return false;
    const c=card(m);const model=c?.querySelector('.dt-reference-3d-model');if(!model)return false;
  ...
  function boot(){
    ...
    if(current?.active&&current?.loaded&&existing?.loaded)return;
    if(current?.active&&current?.loading&&loadingPromise)return;
