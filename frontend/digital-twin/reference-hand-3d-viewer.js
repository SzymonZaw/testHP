(() => {
  ...
  const VIEWER_VERSION = 'reference-glb-safe-30';
  ...
  function markLoaded(token,model,c){if(token!==bootToken||!model||!model.loaded)return;stopTimers();activeModel=model;try{model.dismissPoster?.();model.updateFraming?.();}catch(_){ }state({active:true,loading:false,loaded:true,error:null,loadMethod:'same_origin_fastapi'});const st=c.querySelector('.dt-reference-3d-status');if(st)st.textContent='Loaded NIH GLB · public reference geometry · not user health data';mountLocalSpatialPreview(c.parentElement||mountPoint())}
