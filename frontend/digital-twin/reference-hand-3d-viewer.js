(() => {
  ...
  const VIEWER_VERSION = 'reference-glb-safe-26';
  ...
  function mountPoint(){
    const h=host();
    return h && (
      h.querySelector('#twin-viewport') ||
      h.querySelector('.dt-viewport') ||
      h.querySelector('.center .viewport') ||
      h.querySelector('.viewport') ||
      h
    );
  }
  function styles(){
  ...
  function card(m){
    ...
    c.innerHTML='<model-viewer class="dt-reference-3d-model" alt="NIH 3D healthy adult human hand reference template" loading="eager" reveal="auto" camera-controls touch-action="pan-y" interaction-prompt="none" shadow-intensity="0" exposure="0.95" camera-orbit="0deg 75deg 105%"></model-viewer><div class="dt-reference-3d-overlay"><div class="dt-reference-3d-title">REFERENCE HAND · NIH 3D · 3DPX-017237</div><div class="dt-reference-3d-source">GLB · PUBLIC REFERENCE</div><div class="dt-reference-3d-status">Loading NIH 3D reference geometry…</div><div class="dt-reference-3d-mapping">Region geometry mapping · NOT ESTABLISHED</div></div>';
  ...
  function mount(token){const m=mountPoint();if(!m)return false;let c=m.querySelector(':scope > .dt-reference-3d-card');if(!c && activeModel && activeModel.isConnected===false){const oldCard=activeModel.closest('.dt-reference-3d-card');if(oldCard){m.appendChild(oldCard);c=oldCard;}}
    if(!c)c=card(m);const model=c?.querySelector('.dt-reference-3d-model');if(!model)return false;if(model.loaded){markLoaded(token,model,c);return true}if(model.getAttribute('src')||loadingPromise)return true;loadingPromise=load(token,model,c);return true}
  function boot(){styles();const current=window.__testhpReferenceHand3DViewerState;if(current?.active&&current?.loaded&&activeModel?.isConnected)return;const token=++bootToken;stopTimers();loadingPromise=null;if(current?.active&&current?.loaded&&activeModel&&!activeModel.isConnected){const m=mountPoint();const c=activeModel.closest('.dt-reference-3d-card');if(m&&c){m.appendChild(c);state({active:true,loading:false,loaded:true,error:null,loadMethod:'same_origin_fastapi'});return;}}
