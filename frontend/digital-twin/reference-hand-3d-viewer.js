(() => {
  ...
  const VIEWER_VERSION = 'reference-glb-safe-23';
  ...
  let observer=null,retryTimer=null,bootToken=0,loadingPromise=null,loadPollTimer=null;
  ...
  function markLoaded(token,model,c){if(token!==bootToken||!model)return;stopTimers();state({active:true,loading:false,loaded:true,error:null,loadMethod:'same_origin_fastapi'});const st=c.querySelector('.dt-reference-3d-status');if(st)st.textContent='Loaded NIH GLB · public reference geometry · not user health data';}
