(() => {
  ...
  function installStyles() {
    ...
    s.textContent = `
      ...
      .dt-reference-3d-model{display:block;width:100%;height:520px;background:#0b1118;--poster-color:#0b1118;--progress-bar-color:#79bce9;--progress-mask:transparent}
      ...
      .dt-reference-3d-source{position:absolute;right:16px;top:14px;padding:6px 8px;border:1px solid #2e3d4c;border-radius:8px;background:#0d151ee8;color:#9fb0c2;font:600 10px/1.2 system-ui,sans-serif}
      .dt-reference-3d-mapping{position:absolute;right:16px;bottom:14px;padding:6px 8px;border:1px solid #2e3d4c;border-radius:8px;background:#0d151ee8;color:#8495a6;font:600 10px/1.2 system-ui,sans-serif}
    `;
  ...
  function bindModel(card, token) {
    ...
    model.addEventListener('error', (event) => {
      if (token !== bootToken) return;
      stopWaiting();
      const detail = event?.detail?.sourceError?.message || 'NIH GLB could not be loaded';
