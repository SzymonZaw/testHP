(() => {
  ...
  const VIEWER_VERSION = 'reference-glb-safe-25';
  ...
  function mount(token){const m=mountPoint();if(!m)return false;const c=card(m);const model=c?.querySelector('.dt-reference-3d-model');if(!model)return false;if(model.loaded){markLoaded(token,model,c);return true}if(model.getAttribute('src')||loadingPromise)return true;loadingPromise=load(token,model,c);return true}
  function boot(){styles();const current=window.__testhpReferenceHand3DViewerState;const existing=activeModel||document.querySelector('.dt-reference-3d-model');if(current?.active&&current?.loaded&&existing?.loaded)return;if(current?.active&&current?.loading&&loadingPromise)return;const token=++bootToken;stopTimers();loadingPromise=null;if(activeModel&&!document.contains(activeModel))activeModel=null;state({active:true,loading:true,loaded:false,error:null,loadMethod:'same_origin_fastapi',assetFormat:'glb',assetUrl:NIH_GLB_URL});if(mount(token))return;observer=new MutationObserver(()=>{if(mount(token)){observer.disconnect();observer=null}});observer.observe(document.documentElement||document,{childList:true,subtree:true});let attempts=0;retryTimer=setInterval(()=>{attempts++;if(mount(token)||attempts>=30){clearInterval(retryTimer);retryTimer=null}},250)}
