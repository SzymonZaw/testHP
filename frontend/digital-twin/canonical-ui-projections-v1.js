...
export function selectEvidence(state) {
  ...
  return {
    ...
    missingModalities: Array.isArray(evidence.missingModalities) ? evidence.missingModalities : [],
    qc: { usable, unusable, missing },
    modalities,
...
export function selectHealthHierarchy(state) {
  ...
  return {
    hand: {
      id: 'hand',
      label: 'Whole Hand',
      health: normalizeLevel(health.state),
      confidence: health.confidence ?? null,
      uncertainty: health.uncertainty ?? state?.uncertainty ?? null,
      evidence: health.evidence ?? state?.evidence ?? null,
    },
...
export function selectBiologicalAgeHierarchy(state) {
  ...
  return {
    status: modelStatus(age),
    modelId: age.model_id ?? null,
    modelVersion: age.model_version ?? null,
    levels: levels.map(level => ({
      id: level.id ?? null,
      level: level.level ?? null,
      label: level.label ?? level.level ?? 'Unknown',
      age: level.age ?? null,
      uncertainty: level.uncertainty ?? null,
      confidence: level.confidence ?? null,
      evidence: level.evidence ?? null,
      status: level.status ?? modelStatus(age),
    })),
  };
}
...
export function selectMolecular(state) {
  ...
  states.forEach(item => {
    const key = String(item?.modality ?? '').toLowerCase();
    if (key in byModality) byModality[key] = item;
  });
  return Object.fromEntries(Object.entries(byModality).map(([key, item]) => [key, {
    availability: item?.availability ?? (item ? 'available' : 'missing'),
    qc: item?.qc ?? null,
    features: item?.features ?? [],
    biological_state: item?.biological_state ?? null,
...
export function selectCells(state) {
  ...
  return cells.map(cell => ({
    id: cell.id ?? null,
    cell_type: cell.cell_type ?? cell.type ?? 'unknown',
    location: cell.location ?? null,
    tissue: cell.tissue ?? null,
    count: cell.count ?? null,
    health: cell.health ?? { state: 'unknown' },
    age: cell.age ?? null,
    molecular_state: cell.molecular_state ?? null,
...
export function selectIntervention(state) {
  ...
  return {
    ...
    clinical_validation: value.clinical_validation ?? false,
  };
}
...
export function select3DTwin(state) {
  ...
  return {
    selected,
    health: hierarchy.hand,
    regions: hierarchy.regions.map(region => ({
      id: region.id ?? null,
      label: region.label ?? region.name ?? 'Unknown',
      health: normalizeLevel(region.health?.state ?? region.health),
      confidence: region.health?.confidence ?? region.confidence ?? null,
      uncertainty: region.health?.uncertainty ?? null,
      evidence: region.health?.evidence ?? region.evidence ?? null,
    })),
  };
}
...
export function buildCanonicalViewModel(state) {
  return {
    evidence: selectEvidence(state),
    health: selectHealthHierarchy(state),
    biologicalAge: selectBiologicalAgeHierarchy(state),
    molecular: selectMolecular(state),
    cells: selectCells(state),
    intervention: selectIntervention(state),
    twin3d: select3DTwin(state),
  };
}

window.TestHPCanonicalViews = Object.freeze({
  selectEvidence,
...
window.addEventListener('testhp:canonical-state-changed', event => {
  if (!event.detail || !window.TestHPCanonicalViews) return;
  window.dispatchEvent(new CustomEvent('testhp:canonical-view-model-changed', {
    detail: buildCanonicalViewModel(event.detail),
  }));
});
