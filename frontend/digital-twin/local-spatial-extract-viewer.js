(() => {
  ...
  function ensureStyles() {
    ...
    style.textContent = `
      .${CARD_CLASS}{
        margin-top:16px;
        border:1px solid rgba(155,216,196,.22);
        border-radius:16px;
        background:#0b1118;
        overflow:hidden;
      }
  ...
  function cardRoot(parent) {
    ...
    card.innerHTML = `
      <div class="dt-local-spatial-head">
        <div>
          <div class="dt-local-spatial-kicker">REAL LINKED DATA</div>
          <div class="dt-local-spatial-title">MERFISH · LOCAL SPATIAL EXTRACT</div>
        </div>
        <div class="dt-local-spatial-meta">FOREARM · SAMPLE-LOCAL<br>Not registered to NIH hand geometry</div>
      </div>
  ...
  function draw(canvas, cells) {
    ...
    const points = cells
      .map(cell => Array.isArray(cell?.spatial) ? cell.spatial : null)
      .filter(p => p && p.length >= 2 && Number.isFinite(Number(p[0])) && Number.isFinite(Number(p[1])))
      .slice(0, MAX_POINTS)
    ...
    ctx.strokeStyle = 'rgba(155,216,196,.14)';
    ctx.strokeRect(ox, oy, plotW, plotH);
    ctx.fillStyle = '#9bd8c4';
    for (const [x, y] of points) {
    ...
    ctx.fillText(`n=${points.length}`, 10, height - 10);
  }
  ...
  async function load() {
    ...
    try {
      ...
      draw(card.querySelector('canvas'), cells);
      if (status) {
        ...
        status.textContent = `${payload.returnedCount || cells.length} real cells · ${site} · sample ${sample}`;
      }
      return true;
    } catch (error) {
  ...
  const boot = () => {
    ...
    const timer = window.setInterval(() => {
      attempts += 1;
      if (load() || attempts >= 40) window.clearInterval(timer);
    }, 250);
    window.addEventListener('beforeunload', () => window.clearInterval(timer), { once: true });
  };

  window.testhpLocalSpatialExtract = Object.freeze({ version: 'local-spatial-extract-safe-1', sourceId: SOURCE_ID, load });
