(()=>{
  ...
  async function load(region=currentRegion()) {
    const parent=mountPoint();if(!parent)return false;ensureStyles();const card=cardRoot(parent);bindCanvas(card.querySelector('canvas'),card);const normalizedRegion=String(region||'palm').trim().toLowerCase()||'palm';if(loading&&activeRegion===normalizedRegion)return true;if(loaded&&activeRegion===normalizedRegion)return true;
  ...
  function onCanonicalStateChanged(){const region=currentRegion();if(region!==activeRegion){loaded=false;load(region);}}
  function boot(){let observer,attempts=0;const tryMount=()=>{attempts+=1;const mounted=!!document.querySelector(`.${CARD_CLASS}`)||!!mountPoint();if(mounted){load();if(document.querySelector(`.${CARD_CLASS}`)){observer?.disconnect();observer=undefined;return;}}if(attempts>=240){observer?.disconnect();observer=undefined;return;}window.requestAnimationFrame(tryMount);};observer=new MutationObserver(()=>{if(document.querySelector(`.${CARD_CLASS}`)){observer.disconnect();observer=undefined;return;}load();});observer.observe(document.documentElement||document,{childList:true,subtree:true});tryMount();window.addEventListener('testhp:reference-hand-activated',load);window.addEventListener('testhp:canonical-state-changed',onCanonicalStateChanged);}
  window.testhpLocalSpatialExtract=Object.freeze({version:'local-spatial-extract-safe-5',sourceId:SOURCE_ID,load,getState:()=>({installed:true,loaded,loading,region:activeRegion,selectedCellId,cardPresent:!!document.querySelector(`.${CARD_CLASS}`)})});
