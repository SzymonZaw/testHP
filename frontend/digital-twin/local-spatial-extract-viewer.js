(()=>{
  ...
  async function load(region=currentRegion()) {
    ...
    try{const params=new URLSearchParams({region:normalizedRegion,limit:String(MAX_POINTS)}),response=await fetch(`${ENDPOINT}?${params.toString()}`,{cache:'no-store',credentials:'same-origin',signal:abortController.signal}),payload=await response.json();if(!response.ok)throw new Error(payload?.detail||`local extract endpoint returned ${response.status}`);if(activeRegion!==normalizedRegion)return true;activeCells=Array.isArray(payload?.cells)?payload.cells:[];const count=draw(card.querySelector('canvas'),activeCells),site=payload?.anatomicSite||payload?.region||activeCells[0]?.anatomicSite||normalizedRegion;if(status){if(count>0){status.textContent=`${count} plotted real cells · ${site} · sample-local coordinates`;}else if(payload?.status==='bounded_cell_preview_not_materialized'){status.textContent=`No materialized local cells for ${normalizedRegion}. No remote scan performed.`;}else{status.textContent=`No local cells returned for ${normalizedRegion}.`;}}loaded=true;}catch(error){if(error?.name==='AbortError'){if(status)status.textContent=`Local spatial extract timed out after ${REQUEST_TIMEOUT_MS/1000}s.`;loaded=false;return true;}if(status)status.textContent=`Local spatial extract unavailable: ${error?.message||error}`;loaded=false;}finally{clearTimeout(timeoutId);if(activeRegion===normalizedRegion)loading=false;}return true;
  }
  ...
  window.testhpLocalSpatialExtract=Object.freeze({version:'local-spatial-extract-safe-10',sourceId:SOURCE_ID,load,getState:()=>({installed:true,loaded,loading,region:activeRegion,selectedCellId,cardPresent:!!document.querySelector(`.${CARD_CLASS}`)})});
