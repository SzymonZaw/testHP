(() => {
  ...
  const publish=detail=>{current=normalizeTarget(detail);migrateEvidenceTargets();window.selectedSpatialNode=current;window.spatialEvidenceTarget=current;window.testhpSpatialTarget=current;window.dispatchEvent(new CustomEvent('testhp:spatial-contract-changed',{detail:current}));return current};
