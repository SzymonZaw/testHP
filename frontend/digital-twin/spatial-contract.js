(() => {
  ...
  const LEVELS = new Set(['macro', 'tissue', 'cellular', 'cell']);
  ...
  const relation = (selectedId, candidateId) => {
    ...
    const candidate = normalizeId(candidateId);
  ...
  const scope = (selectedId, ids, includeDescendants = false) =>
    (Array.isArray(ids) ? ids : []).filter(id => inScope(selectedId, id, includeDescendants));

  const normalizeTarget = detail => {
    const source = detail || {};
    const path = Array.isArray(source.path) ? source.path.map(String) : [];
    const spatialId = normalizeId(source.spatial_id || buildSpatialId(path) || source.id || 'hand');
    const segments = spatialId.split('/').filter(Boolean);
    const level = String(source.level || '').toLowerCase() || 'macro';
    const target = {
      spatial_id: spatialId,
      id: normalizeSegment(source.id || segments.at(-1) || 'hand'),
      label: source.target || source.label || path.at(-1) || segments.at(-1) || 'Hand',
      level: LEVELS.has(level) ? level : level === 'single cell' ? 'cell' : level,
  ...
  const publish = detail => {
    ...
    window.selectedSpatialNode = current;
    window.spatialEvidenceTarget = current;
    window.testhpSpatialTarget = current;
  ...
  window.addEventListener('testhp:spatial-contract-request', event => {
    event?.detail?.callback?.(current);
  });
