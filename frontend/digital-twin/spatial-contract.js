(() => {
  ...
  const SEGMENT_ALIASES=Object.freeze({'hypothenar-eminence':'hypothenar','thenar-eminence':'thenar'});
  const normalizeSegment=value=>{const segment=String(value??'').trim().toLowerCase().replaceAll(' ','-').replace(SEGMENT_RE,'-');return SEGMENT_ALIASES[segment]||segment};
  ...
  const migrateEvidenceTargets=()=>{try{const key='digitalTwinEvidenceUX.v2';const raw=JSON.parse(localStorage.getItem(key)||'{}');if(!Array.isArray(raw.evidence))return;let changed=false;raw.evidence=raw.evidence.map(item=>{if(!item||typeof item!=='object')return item;const next={...item};const source=next.target??next.spatial_id??next.spatialId;const normalized=normalizeId(source);if(normalized&&normalized!==source){next.target=normalized;changed=true}return next});if(changed)localStorage.setItem(key,JSON.stringify(raw))}catch{}}
  ...
  const publish=detail=>{current=normalizeTarget(detail);migrateEvidenceTargets();window.selectedSpatialNode=current.spatial_id;window.spatialEvidenceTarget=current.spatial_id;window.testhpSpatialTarget=current.spatial_id;window.dispatchEvent(new CustomEvent('testhp:spatial-contract-changed',{detail:current}));return current};
  window.testhpSpatialContract=Object.freeze({normalizeId,buildSpatialId,relation,inScope,scope,getTarget:()=>current,publish,LEVELS:[...LEVELS],SEGMENT_ALIASES});
