...
export function normalizeQCResult(value) {
  ...
  return {
    modality: value?.modality ?? 'unknown',
    status,
...
export function normalizeValidationRecord(value) {
  return {
    model_id: value?.model_id ?? null,
    training_dataset: value?.training_dataset ?? null,
    validation_dataset: value?.validation_dataset ?? null,
    test_dataset: value?.test_dataset ?? null,
    metrics: value?.metrics && typeof value.metrics === 'object' ? value.metrics : {},
    population: value?.population ?? null,
    tissue: value?.tissue ?? null,
    cell_type: value?.cell_type ?? null,
