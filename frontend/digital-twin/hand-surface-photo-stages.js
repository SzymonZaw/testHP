(() => {
  ...
  function stagePanel() {
    ...
    if (!card) return null;
    let controls = $('p3r-stage-1-5');
    if (controls) return controls;
    controls = document.createElement('div');
    controls.id = 'p3r-stage-1-5';
    controls.innerHTML = `
      <div class="p3r-upload">
        <label class="primary p3r-upload-label" for="p3r-photo-files">Add photographs</label>
        <input id="p3r-photo-files" type="file" accept="image/jpeg,image/png,image/webp,image/tiff" multiple>
        <button id="p3r-register" type="button">Register prepared views</button>
      </div>
      <div id="p3r-stage-summary" class="p3r-stage-summary"></div>
      <div id="p3r-stage-list" class="p3r-list"></div>
      <div id="p3r-stage-message" class="p3r-status" style="margin-top:10px">Loading photo inputs…</div>`;
    const oldList = $('p3r-inputs');
    oldList?.replaceWith(controls.querySelector('#p3r-stage-list'));
    card.querySelector('.p3r-note')?.insertAdjacentElement('afterend', controls.querySelector('.p3r-upload'));
    card.querySelector('.p3r-meter')?.insertAdjacentElement('beforebegin', controls.querySelector('#p3r-stage-summary'));
    card.querySelector('.p3r-meter')?.insertAdjacentElement('beforebegin', controls.querySelector('#p3r-stage-list'));
    card.querySelector('.p3r-status')?.insertAdjacentElement('beforebegin', controls.querySelector('#p3r-register'));
    // The elements above are moved out of `controls`; keep the message in the card.
    const message = document.createElement('div');
    message.id = 'p3r-stage-message';
    message.className = 'p3r-status';
  ...
  function ensureControls() {
    ...
    const message = controls.querySelector('#p3r-stage-message');
    ...
    card.querySelector('#p3r-status')?.replaceWith(message);
    card.querySelector('#p3r-meta')?.style.setProperty('display', 'none');
    const input = $('p3r-photo-files');
  ...
  async function uploadFiles(files) {
    ...
    try {
      ...
      message(error.message || 'Upload failed.', true);
    } finally {
      const input = $('p3r-photo-files');
  ...
  async function registerViews() {
    const t = target();
    if (!current || current.prepared_count < 2) {
      message('Prepare at least two assigned views before registration.', true);
      return;
    }
    try {
  ...
  function render() {
    ...
    list.querySelectorAll('select[data-asset]').forEach(select => select.addEventListener('change', () => {
      if (select.value) assign(select.dataset.asset, select.value);
    }));
  ...
  async function syncLegacyEvidence() {
    ...
    try {
      ...
      for (const item of current.inputs.filter(x => x.prepared && x.prepared_asset_id && x.view)) {
        ...
        prepared.push({
          evidence_id: `photo-prepared-${item.prepared_asset_id}`,
          asset_id: item.asset_id,
          sourceType: 'prepared-image',
          prepared: true,
          filename: item.filename,
          view: item.view,
          preparedAsset: { name: item.filename, view: item.view, dataUrl },
      ...
      // The server manifest remains canonical; the legacy cache is only a compatibility bridge.
