(() => {
  ...
  const render = target => {
    ensurePanel();
    const panel = document.getElementById('cell-timeline-panel');
    if (!panel) return;
    ...
    panel.hidden = !timeline;
    if (!timeline) return;
    const points = document.getElementById('cell-timeline-points');
    points.innerHTML = timeline.points.map(point => `<div class="assessment-metric" style="margin-bottom:6px"><span>${point.timepoint}</span><strong>Wiek ${point.biologicalAge ?? '—'} · abnormality ${point.abnormality ?? '—'}</strong><span>Health ${point.healthScore ?? '—'} · uncertainty ${point.uncertainty ?? '—'}</span></div>`).join('');
