...
if(viewport&&badge&&node&&children){
  ...
  const scene=new THREE.Scene();scene.background=new THREE.Color(0x0b1518);
  ...
  function renderTissue(){
    ...
    const positions=[[-2.05,.55,.25],[0,-.25,.45],[2.05,.55,.25]];const targets=targetElements();
    (targets.length?targets:[null,null,null]).slice(0,3).forEach((t,i)=>{const m=mesh(new THREE.BoxGeometry(1.72,1.35,.28),positions[i],t,COLORS.tissue);m.rotation.z=(i-1)*.05;if(t)addLabel(targetText(t),23+i*27,59-(i%2)*18,t)});
  ...
  function renderCell(){
    const t=targetElements()[0];const outer=new THREE.Mesh(new THREE.SphereGeometry(1.45,48,32),new THREE.MeshStandardMaterial({color:COLORS.cell,roughness:.55,transparent:true,opacity:.82,emissive:0x0b3026,emissiveIntensity:.35}));outer.position.z=.1;if(t){outer.userData.target=t;clickable.push(outer)}root.add(outer);
    const nucleus=new THREE.Mesh(new THREE.SphereGeometry(.55,40,24),new THREE.MeshStandardMaterial({color:0x315e51,roughness:.45,emissive:0x183b31,emissiveIntensity:.45}));nucleus.position.set(-.2,.1,1.05);root.add(nucleus);if(t)addLabel(targetText(t),50,82,t);
  }
  ...
  function navigateToLayer(requested){
    if(requested==='macro'){const b=[...document.querySelectorAll('#spatial-breadcrumb button')];(b[0]||b[1])?.click();return;}
    if(requested===level())return;
    let guard=0;while(level()!==requested&&guard++<4){const next=targetElements()[0];if(!next)break;next.click();}
  }
  function renderSwitcher(){
    ...
    const current=level();
    layerOrder.forEach(layer=>{const b=document.createElement('button');b.type='button';b.textContent=layerNames[layer];Object.assign(b.style,{padding:'7px 10px',borderRadius:'8px',border:'1px solid '+(layer===current?'#8db7aa':'#36544e'),background:layer===current?'#23463e':'#101b1a',color:layer===current?'#fff':'#cbdad5',font:'700 10px system-ui,sans-serif',cursor:'pointer'});b.onclick=()=>navigateToLayer(layer);switcher.appendChild(b)});
  }
  function render(){
    const l=level(),t=title(),deep=l!=='macro',k=`${l}:${t}:${targetElements().map(targetText).join('|')}`;
    renderSwitcher();syncBoundary(deep);layerCanvas.style.display=deep?'block':'none';labels.style.display=deep?'block':'none';
    if(baseCanvas)baseCanvas.style.visibility=deep?'hidden':'visible';if(controls)controls.style.visibility=deep?'hidden':'visible';if(hint)hint.style.visibility=deep?'hidden':'visible';if(loading)loading.style.visibility=deep?'hidden':'visible';
    if(!deep)return;if(k===last){resize();return}last=k;clear();camera.position.set(0,0,8);camera.lookAt(0,0,0);if(l==='tissue')renderTissue();else if(l==='cellular')renderCellular();else renderCell();resize();
  }
  ...
  const navigator=document.querySelector('.spatial-navigator');const breadcrumb=document.getElementById('spatial-breadcrumb');if(navigator&&breadcrumb)navigator.insertBefore(switcher,breadcrumb);
  new MutationObserver(render).observe(badge,{childList:true,characterData:true,subtree:true});new MutationObserver(render).observe(node,{childList:true,characterData:true,subtree:true});new MutationObserver(render).observe(children,{childList:true,characterData:true,subtree:true});if(breadcrumb)new MutationObserver(render).observe(breadcrumb,{childList:true,subtree:true,characterData:true});window.addEventListener('resize',resize);render();animate();
}
