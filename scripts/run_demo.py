...
def run() -> int:
    ...
    for index, timepoint in enumerate(sorted(by_timepoint)):
        ...
        rejected.extend(r for r in items if r.observation.quality_score < 0.5)
        pipeline.ingest(
            timepoint,
    ...
    inflammation_key = "blood:systemic:inflammation_marker:mg_L"
    bone_key = "mri:lumbar_spine:bone_density:g_cm3"
    alt_bone_key = "mri:lumbar_spine:bone_density_alt:g_cm3"
    ...
    assert "microscopy:skin:cell_size:um" not in t4
    print("[6] Low-quality observation exclusion    PASS")
    print("    T4 microscopy observation excluded")

    assert bone_key in t5 and alt_bone_key in t5 and t5[bone_key] != t5[alt_bone_key]
